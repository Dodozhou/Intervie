...
12. 多态的三个条件
    ...
    * 覆盖父类方法
    * 向上转型    
13. 在接口里面的变量默认都是public static final 的，它们是公共的,静态的,最终的常量.相当于全局常量，可以直接省略修饰符。实现类可以直接访问接口中的变量
14. try-catch-finally
    [详情见这里](http://blog.51cto.com/qing0991/1387200)
    * 总结
    1. return语句并不是函数的最终出口，如果有finally语句，这在return之后还会执行finally（return的值会暂存在栈里面，等待finally执行后再返回
    2.  finally里面不建议放return语句，根据需要，return语句可以放在try和catch里面和函数的最后。可行的做法有四：
...
2. 请描述java的内存分区  
 ...
 当线程终止时，虚拟机栈，本地方法栈，程序计算器所占用的内存空间会被释放掉
...
6. [volatile关键字](http://www.importnew.com/18126.html)(内容太多了，详细见这)
